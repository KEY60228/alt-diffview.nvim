local ffi = require("alt-diffview.ffi")
...
local function dstring(object)
  if not AltDiffviewGlobal.logger then return "" end
  dstring = AltDiffviewGlobal.logger.dstring
...
---@class Waitable : alt-diffview.Object
...
function Future:dprint(...)
  if not AltDiffviewGlobal.logger then return end
  if AltDiffviewGlobal.debug_level >= 10 or M._watching[self] then
    local t = { self, "::", ... }
    for i = 1, table.maxn(t) do t[i] = dstring(t[i]) end
    AltDiffviewGlobal.logger:debug(table.concat(t, " "))
  end
end

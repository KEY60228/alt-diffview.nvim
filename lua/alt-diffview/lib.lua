...
local DiffView = lazy.access("alt-diffview.scene.views.diff.diff_view", "DiffView") ---@type DiffView|LazyModule
local FileHistoryView = lazy.access("alt-diffview.scene.views.file_history.file_history_view", "FileHistoryView") ---@type FileHistoryView|LazyModule
...
function M.diffview_open(args)
  local default_args = config.get_config().default_args.AltDiffviewOpen
  local argo = arg_parser.parse(utils.flatten({ default_args, args }))
  local rev_arg = argo.args[1]

  logger:info("[command call] :AltDiffviewOpen " .. table.concat(utils.flatten({
    default_args,
    args,
  }), " "))
...
function M.file_history(range, args)
  local default_args = config.get_config().default_args.AltDiffviewFileHistory
  local argo = arg_parser.parse(utils.flatten({ default_args, args }))

  logger:info("[command call] :AltDiffviewFileHistory " .. table.concat(utils.flatten({
    default_args,
    args,
  }), " "))
...
---Get the currently open AltDiffview.
---@return View?

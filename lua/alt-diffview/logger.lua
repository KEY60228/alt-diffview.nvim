...
local Mock = lazy.access("alt-diffview.mock", "Mock") ---@type Mock|LazyModule
local Semaphore = lazy.access("alt-diffview.control", "Semaphore") ---@type Semaphore|LazyModule
local loop = lazy.require("alt-diffview.debounce") ---@module "alt-diffview.debounce"
...
---@class Logger : alt-diffview.Object
...
function Logger:init(opt)
  ...
  self.level = AltDiffviewGlobal.debug_level > 0 and Logger.LogLevels.debug or Logger.LogLevels.info
  self.msg_buffer = {}
...
function Logger:lvl(min_level)
  if AltDiffviewGlobal.debug_level >= min_level then
    return self
  end
...
---@param job alt-diffview.Job
---@param opt? Logger.log_job.Opt
function Logger:log_job(job, opt)
  opt = opt or {}

  if opt.silent then return end
  if opt.debug_level and AltDiffviewGlobal.debug_level < opt.debug_level then
    return
  end

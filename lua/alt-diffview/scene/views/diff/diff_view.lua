...
local CommitLogPanel = lazy.access("alt-diffview.ui.panels.commit_log_panel", "CommitLogPanel") ---@type CommitLogPanel|LazyModule
local Diff = lazy.access("alt-diffview.diff", "Diff") ---@type Diff|LazyModule
local EditToken = lazy.access("alt-diffview.diff", "EditToken") ---@type EditToken|LazyModule
local EventName = lazy.access("alt-diffview.events", "EventName") ---@type EventName|LazyModule
local FileDict = lazy.access("alt-diffview.vcs.file_dict", "FileDict") ---@type FileDict|LazyModule
local FileEntry = lazy.access("alt-diffview.scene.file_entry", "FileEntry") ---@type FileEntry|LazyModule
local FilePanel = lazy.access("alt-diffview.scene.views.diff.file_panel", "FilePanel") ---@type FilePanel|LazyModule
local PerfTimer = lazy.access("alt-diffview.perf", "PerfTimer") ---@type PerfTimer|LazyModule
local RevType = lazy.access("alt-diffview.vcs.rev", "RevType") ---@type RevType|LazyModule
local StandardView = lazy.access("alt-diffview.scene.views.standard.standard_view", "StandardView") ---@type StandardView|LazyModule
local config = lazy.require("alt-diffview.config") ---@module "alt-diffview.config"
local debounce = lazy.require("alt-diffview.debounce") ---@module "alt-diffview.debounce"
local utils = lazy.require("alt-diffview.utils") ---@module "alt-diffview.utils"
local vcs_utils = lazy.require("alt-diffview.vcs.utils") ---@module "alt-diffview.vcs.utils"
local GitAdapter = lazy.access("alt-diffview.vcs.adapters.git", "GitAdapter") ---@type GitAdapter|LazyModule
...
function DiffView:post_open()
  ...
  self.commit_log_panel = CommitLogPanel(self.adapter, {
    name = fmt("alt-diffview://%s/log/%d/%s", self.adapter.ctx.dir, self.tabpage, "commit_log"),
  })
...
DiffView._set_file = async.void(function(self, file)
  ...
  if not self.cur_entry.opened then
    ...
    AltDiffviewGlobal.emitter:emit("file_open_new", file)
  end
end)
...
function DiffView:init_event_listeners()
  local listeners = require("alt-diffview.scene.views.diff.listeners")(self)

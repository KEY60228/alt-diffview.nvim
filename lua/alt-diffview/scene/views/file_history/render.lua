...
local function render_files(comp, files)
  for i, file in ipairs(files) do
    comp:add_text(i == #files and "└   " or "│   ", "AltDiffviewNonText")

    if file:is_null_entry() then
      comp:add_text(
        "No diff",
        file.active and "AltDiffviewFilePanelSelected" or "AltDiffviewFilePanelFileName"
      )
    else
      if file.status then
        comp:add_text(file.status .. " ", hl.get_git_hl(file.status))
      else
        comp:add_text("-" .. " ", "AltDiffviewNonText")
      end
  ...
  for i, entry in ipairs(entries) do
    ...
    if not entry.single_file then
      ...
      if entry.nulled then
        comp:add_text(utils.str_center_pad("empty", #s_num_files + 7), "AltDiffviewFilePanelCounter")
      else
        comp:add_text(
          fmt(
            " %s file%s",
          ...
          "AltDiffviewFilePanelCounter"
        )
      end
    end

    if max_len_stats ~= -1 then
      local adds = { "-", "AltDiffviewNonText" }
      local dels = { "-", "AltDiffviewNonText" }

      if entry.stats and entry.stats.additions then
        adds = { tostring(entry.stats.additions), "AltDiffviewFilePanelInsertions" }
      end

      if entry.stats and entry.stats.deletions then
        dels = { tostring(entry.stats.deletions), "AltDiffviewFilePanelDeletions" }
      end
    ...
    if entry.commit.hash then
      comp:add_text(" " .. entry.commit.hash:sub(1, 8), "AltDiffviewHash")
    end

    if (entry.commit --[[@as GitCommit ]]).reflog_selector then
      comp:add_text((" %s"):format((entry.commit --[[@as GitCommit ]]).reflog_selector), "AltDiffviewReflogSelector")
    end

    if entry.commit.ref_names then
      comp:add_text((" (%s)"):format(entry.commit.ref_names), "AltDiffviewReference")
    end
    ...
    comp:add_text(
      " " .. subject,
      panel.cur_item[1] == entry and "AltDiffviewFilePanelSelected" or "AltDiffviewFilePanelFileName"
    )
    ...
    if entry.commit then
      ...
      comp:add_text(" " .. entry.commit.author .. ", " .. date, "AltDiffviewFilePanelPath")
    end
...
return {
  ...
  file_history_panel = function(panel)
    ...
    -- root path
    comp:add_text(cached.root_path, "AltDiffviewFilePanelRootPath")
    comp:ln()
    ...
    if panel.single_file then
      if #panel.entries > 0 then
        ...
        comp:add_text(file.basename, "AltDiffviewFilePanelFileName")
        comp:ln()
      end
    elseif #cached.args > 0 then
      comp:add_text("Showing history for: ", "AltDiffviewFilePanelPath")
      comp:add_text(cached.args, "AltDiffviewFilePanelFileName")
      comp:ln()
    end

    if log_options.rev_range and log_options.rev_range ~= "" then
      comp:add_text("Revision range: ", "AltDiffviewFilePanelPath")
      comp:add_text(log_options.rev_range, "AltDiffviewFilePanelFileName")
      comp:ln()
    end

    if panel.option_mapping then
      comp:add_text("Options: ", "AltDiffviewFilePanelPath")
      comp:add_text(panel.option_mapping, "AltDiffviewFilePanelCounter")
      comp:ln()
    end

    if conf.show_help_hints and panel.help_mapping then
      comp:add_text("Help: ", "AltDiffviewFilePanelPath")
      comp:add_text(panel.help_mapping, "AltDiffviewFilePanelCounter")
      comp:ln()
    end
    ...
    comp:add_line()
    comp:add_text("File History ", "AltDiffviewFilePanelTitle")
    comp:add_text("(" .. #panel.entries .. ")", "AltDiffviewFilePanelCounter")

    if panel.updating then
      comp:add_text(" (Updating...)", "AltDiffviewDim1")
    end
  ...
  fh_option_panel = function(panel)
    ...
    comp:add_line("Switches", "AltDiffviewFilePanelTitle")
    ...
    for _, item in ipairs(panel.components.switches.items) do
      ...
      comp:add_text(" " .. option.keymap .. " ", "AltDiffviewSecondary")
      comp:add_text(option.desc .. " (", "AltDiffviewFilePanelFileName")
      comp:add_text(option.flag_name, enabled and "AltDiffviewFilePanelCounter" or "AltDiffviewDim1")
      comp:add_text(")", "AltDiffviewFilePanelFileName")
      comp:ln()
    end

    comp = panel.components.options.title.comp
    comp:add_line()
    comp:add_line("Options", "AltDiffviewFilePanelTitle")
    ...
    for _, item in ipairs(panel.components.options.items) do
      ...
      comp:add_text(display_value, not empty and "AltDiffviewFilePanelCounter" or "AltDiffviewDim1")

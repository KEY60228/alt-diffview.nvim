...
local Diff3Ver = lazy.access("alt-diffview.scene.layouts.diff_3_ver", "Diff3Ver") ---@type Diff3Ver|LazyModule
...
local File = lazy.access("alt-diffview.vcs.file", "File") ---@type vcs.File|LazyModule
...
---@class View : alt-diffview.Object
...
function View:init(opt)
  ...
  local function wrap_event(event)
    AltDiffviewGlobal.emitter:on(event, function(_, view, ...)
      local cur_view = require("alt-diffview.lib").get_current_view()
...
function View:open()
  ...
  self:init_layout()
  self:post_open()
  AltDiffviewGlobal.emitter:emit("view_opened", self)
  AltDiffviewGlobal.emitter:emit("view_enter", self)
end
...
function View:close()
  ...
  if self.tabpage and api.nvim_tabpage_is_valid(self.tabpage) then
    AltDiffviewGlobal.emitter:emit("view_leave", self)
  ...
  AltDiffviewGlobal.emitter:emit("view_closed", self)
end

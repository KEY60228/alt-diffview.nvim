...
local Commit = require("alt-diffview.vcs.adapters.git.commit").GitCommit
...
local GitRev = require("alt-diffview.vcs.adapters.git.rev").GitRev
...
function GitAdapter:stream_fh_data(state)
  ---@type alt-diffview.Job, AsyncListStream
  local job, stream
  ---@type string[]?
  local data
          ...
          utils.warn(
            fmt(
              "Encountered malformed data while parsing commit '%s'!"
                ...
                .. " Call :AltDiffviewRefresh to try again. See :AltDiffviewLog for details.",
              rev_arg:sub(1, 10)
            ),
            true
          )

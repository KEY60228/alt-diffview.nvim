local AsyncListStream = require("alt-diffview.stream").AsyncListStream
local Commit = require("alt-diffview.vcs.adapters.hg.commit").HgCommit
local Diff2Hor = require("alt-diffview.scene.layouts.diff_2_hor").Diff2Hor
local FileEntry = require("alt-diffview.scene.file_entry").FileEntry
local FlagOption = require("alt-diffview.vcs.flag_option").FlagOption
local HgRev = require('alt-diffview.vcs.adapters.hg.rev').HgRev
local Job = require("alt-diffview.job").Job
local MultiJob = require("alt-diffview.multi_job").MultiJob
local JobStatus = require('alt-diffview.vcs.utils').JobStatus
local LogEntry = require("alt-diffview.vcs.log_entry").LogEntry
local RevType = require("alt-diffview.vcs.rev").RevType
local VCSAdapter = require('alt-diffview.vcs.adapter').VCSAdapter
local arg_parser = require("alt-diffview.arg_parser")
...
local vcs_utils = require("alt-diffview.vcs.utils")
  ...
  for _, item in in_stream:iter() do
    ...
    then
      ...
      if #new_data.namestat == 0 then
        ...
        utils.warn("Displayed history may be incomplete. Check ':AltDiffviewLog' for details.", true)
        out_stream:push({
          JobStatus.ERROR,
          nil,
...
function HgAdapter:show_untracked(opt)
  ...
  local out = self:exec_sync(
    { "log", "--rev=0", "--template={configbool('alt-diffview.nvim', 'untracked', 'True')}" },

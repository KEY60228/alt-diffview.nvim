...
local GitRev = lazy.access("alt-diffview.vcs.adapters.git.rev", "GitRev") ---@type GitRev|LazyModule
...
---@class vcs.File : alt-diffview.Object
...
function File:init(opt)
  ...
  self.winopts = opt.winopts or {
    ...
    foldenable = true,
    winhl = {
      "DiffAdd:AltDiffviewDiffAdd",
      "DiffDelete:AltDiffviewDiffDelete",
      "DiffChange:AltDiffviewDiffChange",
      "DiffText:AltDiffviewDiffText",
    },
  }
...
function File:post_buf_created()
  local view = require("alt-diffview.lib").get_current_view()

  if view then
    view.emitter:on("diff_buf_win_enter", function(_, bufnr, winid, ctx)
      if bufnr == self.bufnr then
        api.nvim_win_call(winid, function()
          AltDiffviewGlobal.emitter:emit("diff_buf_read", self.bufnr, ctx)
        end)
...
File.create_buffer = async.wrap(function(self, callback)
  ...
  local fullname = pl:join("alt-diffview://", self.adapter.ctx.dir, context, self.path)
...
function File._get_null_buffer()
  if not api.nvim_buf_is_loaded(File.NULL_FILE.bufnr or -1) then
    ...
    local bufname = "alt-diffview://null"

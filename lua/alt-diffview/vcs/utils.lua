local FileDict = require("alt-diffview.vcs.file_dict").FileDict
local RevType = require("alt-diffview.vcs.rev").RevType
local Scanner = require("alt-diffview.scanner")
local Semaphore = require("alt-diffview.control").Semaphore
...
M.restore_file = async.void(function(adapter, path, kind, commit)
  ...
  if not ok then
    utils.err("Failed to revert file! See ':AltDiffviewLog' for details.", true)
    return
  end

...
local logger = lazy.access(_G, { "AltDiffviewGlobal", "logger" }) ---@type Logger
...
---@type PathLib
M.path = lazy.require("alt-diffview.path", function(module)
...
function M.notify(msg, level, schedule)
  ...
  vim.notify(msg, level, { title = "alt-diffview.nvim" })
end
...
function M.input(prompt, opt)
  ...
  if type(completion) == "function" then
    AltDiffviewGlobal.state.current_completer = completion
    completion = "customlist,AltDiffview__ui_input_completion"
  end

...
local File = require("alt-diffview.vcs.file").File
local PerfTimer = require("alt-diffview.perf").PerfTimer
local oop = require("alt-diffview.oop")
local renderer = require("alt-diffview.renderer")
...
---@class Panel : alt-diffview.Object
...
Panel.au = {
  ---@type integer
  group = api.nvim_create_augroup("alt_diffview_panels", {}),
  ---@type EventEmitter
...
function Panel:init(opt)
  ...
  self.state = {}
  self.bufname = opt.bufname or "AltDiffviewPanel"
  self.au_event_map = {}
end
...
function Panel:open()
  ...
  elseif config.type == "float" then
    ...
    if self.winid == 0 then
      self.winid = nil
      error("[alt-diffview.nvim] Failed to open float panel window!")
    end
  end
...
function Panel:init_buffer()
  ...
  if pl:is_abs(self.bufname) or pl:is_uri(self.bufname) then
    ...
    bufname = string.format("alt-diffview:///panels/%d/%s", Panel.next_uid(), self.bufname)
  end
